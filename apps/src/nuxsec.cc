...
std::filesystem::path resolve_driver_path(const std::string &driver_name)
{
    ...
    if (!ec)
    {
        const auto candidate = exe.parent_path() / driver_name;
...
int run_driver_command(const std::string &driver_name,
                       const std::vector<std::string> &args)
{
    std::ostringstream cmd;
    cmd << shell_quote(resolve_driver_path(driver_name).string());
...
int run_art_command(const std::vector<std::string> &args)
{
    ...
    return run_driver_command("nuxsecArtFileIOdriver", args);
}
...
int run_sample_command(const std::vector<std::string> &args)
{
    ...
    return run_driver_command("nuxsecSampleIOdriver", args);
}
...
int run_event_command(const std::vector<std::string> &args)
{
    ...
    return run_driver_command("nuxsecEventIOdriver", args);
}
