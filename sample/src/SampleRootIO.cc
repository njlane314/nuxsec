 ...
 *
 *  @brief Implementation for SampleRootIO ROOT serialization helpers.
 */
...
void SampleRootIO::write(const Sample &sample, const std::string &out_file)
{
    ...
    {
        TTree fragments("fragments", "ArtIO fragments included in sample aggregation");
...
Sample SampleRootIO::read(const std::string &in_file)
{
    ...
    if (!tree)
    {
        throw std::runtime_error("Missing fragments tree in SampleIO directory");
    }
