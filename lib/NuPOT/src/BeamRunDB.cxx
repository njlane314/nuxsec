...
BeamRunDB::BeamRunDB(std::string path)
...
nuio::DBSums BeamRunDB::SumRuninfoForSelection(const std::vector<nuio::RunSubrun>& pairs) const {
...
void BeamRunDB::Exec(const std::string& sql) const {
  char* err = nullptr;
...
void BeamRunDB::Prepare(const std::string& sql, sqlite3_stmt** stmt) const {
