...
//
// Run with:
//   ./heron macro plotPREffVsLambdaKinematics.C
//   ./heron macro plotPREffVsLambdaKinematics.C 'plotPREffVsLambdaKinematics("./scratch/out/event_list_myana.root","sel_triggered_slice")'
//   ./heron macro plotPREffVsLambdaKinematics.C 'plotPREffVsLambdaKinematics("./scratch/out/event_list_myana.root","sel_triggered_slice")'
//   ./heron macro plotPREffVsLambdaKinematics.C 'plotPREffVsLambdaKinematics("./scratch/out/event_list_myana.root","sel_triggered_slice", "pr_valid_assignment && pr_mu_completeness>0.2 && pr_mu_purity>0.7 && pr_p_completeness>0.2 && pr_p_purity>0.7 && pr_pi_completeness>0.2 && pr_pi_purity>0.7")'
//
// Output:
...
{
    return heron::macro::require_columns(columns, required, "plotPREffVsLambdaKinematics", label);
}
                                    ...
                                    " && (pi_p>0.0)")
{
  return heron::macro::run_with_guard("plotPREffVsLambdaKinematics", [&]() -> int {
    ...
    if (use_imt)
    {
        ...
        std::cout << "[plotPREffVsLambdaKinematics] implicit MT enabled via HERON_ENABLE_IMT\n";
    }
    else
    {
        ROOT::DisableImplicitMT();
        std::cout << "[plotPREffVsLambdaKinematics] implicit MT disabled (set HERON_ENABLE_IMT=1 to enable)\n";
    }
